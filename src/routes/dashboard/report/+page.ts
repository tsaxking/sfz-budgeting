...
export const load = async (event) => {
    ...
    return {
        ...
        to: event.data.to,
        buckets: new DataArr(Transactions.Buckets, event.data.buckets.map(b => Transactions.Buckets.Generator(b))),
        startBalance: event.data.startBalance,
        endBalance: event.data.endBalance,
        tags: new DataArr(Transactions.Tags, event.data.tags.map(t => Transactions.Tags.Generator(t))),
        // budgets,
    }
};