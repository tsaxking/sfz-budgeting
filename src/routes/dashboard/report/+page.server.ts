...
export const load = async (event) => {
    ...
    const buckets = await Transactions.Buckets.all({ type: 'stream', })
        .await()
        .unwrap();

    const transactions = resolveAll(
        await Promise.all(
            buckets.map(b => Transactions.transactionsFromBucket(b))
        )
    ).unwrap().flat();
    ...
    const startBalance = fromTransactions.reduce((acc, t) => {
        return acc - t.transaction.data.amount;
    }, currentBalance) / 100;

    const endBalance = afterTransactions.reduce((acc, t) => {
        return acc - t.transaction.data.amount;
    }, startBalance) / 100;
    ...
    return {
        transactions: transactions
            .filter(t => new Date(t.transaction.data.date) >= new Date(from) && new Date(t.transaction.data.date) <= new Date(to))
            .sort((a, b) => new Date(a.transaction.data.date).getTime() - new Date(b.transaction.data.date).getTime())
            .map(t => ({
