<script lang="ts">
	...
	const saveTags = async () => {
		...
		for (const tagId of Array.from(selectedTags).filter(Boolean)) {
			promises.push(
				Transactions.TransactionTags.new({
					tagId: tagId,
...
<div class="container-fluid">
				...
				<li class="list-item">
					<div class="d-flex">
						<input
							type="checkbox"
							class="form-check"
							id="transaction-tag-{tag.data.id}"
							autocomplete="off"
							checked={selectedTags.has(tag.data.id)}
							onchange={(event) => {
	...
	<div class="row mb-3">
		<input
			type="checkbox"
			class="btn-check"
			id="transaction-reviewed-{$transaction.id}"
			autocomplete="off"
			bind:checked={reviewed}
		/>
		<label class="btn btn-outline-success" for="transaction-reviewed-{$transaction.id}"
			>Reviewed</label
		>
	</div>
</div>
