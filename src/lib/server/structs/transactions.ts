...
export namespace Transactions {
	...
	CSVImports.on('create', (csvImport) => {
		...
		contents.pipe(async (record) => {
			...
			const exists = await Transactions.fromProperty('bucketId', csvImport.data.bucketId, {
				type: 'stream'
			}).await();
			if (exists.isOk()) {
				const find = exists.value.find((t) => {
					...
					return (
						date.getTime() === record.date.getTime() &&
						t.data.description === record.description &&
						record.amount === t.data.amount
					);
				});
				if (find) {
					// already exists
	...
	CSVImports.on('archive', (csvImport) => {
		Transactions.fromProperty('importId', csvImport.id, { type: 'stream' }).pipe((t) =>
			t.setArchive(true)
		);
	});
	...
	export const getTags = (transaction: TransactionData) => {
		return TransactionTags.fromProperty('transactionId', transaction.id, {
			type: 'stream'
		}).await();
	};
	...
	export const transactionsFromBucket = (bucket: BucketData) => {
		return attemptAsync(async () => {
			const transactions = await Transactions.fromProperty('bucketId', bucket.id, {
				type: 'stream'
			})
				.await()
				.unwrap();
		...
		return attemptAsync(async () => {
			const res = await DB.select()
				...
				.innerJoin(
					...
					eq(TransactionTags.table.transactionId, Transactions.table.id)
				)
